...
class EmailAddressGen:
    def __init__(
        self,
        locale: str = "US",
    ):
        self.email_domains_under_30 = EMAIL_DOMAINS_MAP[locale][
            "email_domains_under_30"
        ]
        self.email_domains_30_50 = EMAIL_DOMAINS_MAP[locale]["email_domains_30_50"]
        self.email_domains_over_50 = EMAIL_DOMAINS_MAP[locale]["email_domains_over_50"]
    ...
    def generate_email_address(
        ...
        domain = self.get_email_domain_by_age(age)
        username_base = self.get_email_basename_by_name(
            first_name, middle_name, last_name
        )
        suffix = self.get_email_suffix_by_birth_date(birth_date)

        # Combine to form email
        return f"{username_base}{suffix}@{domain}"

    def get_email_domain_by_age(self, age: int) -> str:
        """
        ...
        if age < 30:
            return random.choices(
                ...
                weights=list(self.email_domains_under_30.values()),
                k=1,
            )[0]
        elif age < 50:
            return random.choices(
                list(self.email_domains_30_50.keys()),
                weights=list(self.email_domains_30_50.values()),
                k=1,
            )[0]
        else:
            return random.choices(
                list(self.email_domains_over_50.keys()),
                weights=list(self.email_domains_over_50.values()),
                k=1,
            )[0]

    def get_email_basename_by_name(
        self, first_name: str, middle_name: str | None, last_name: str
    ) -> str:
        """
    ...
    def get_email_suffix_by_birth_date(self, birth_date: date) -> str:
        """
